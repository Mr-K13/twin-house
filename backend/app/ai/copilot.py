"""
AI Operations Copilot (spec 1️⃣6️⃣)

  question + summarize_state(TwinState) → LLM (OpenAI, structured output) → {text, citations[]}

- With OPENAI_API_KEY: use OpenAI Chat Completions with an enforced JSON reply schema. Citations can only be ids that occur in the summary (events E..., robots R..., tasks A..., decisions D...).
- Without a key (or when the call fails): fall back to the rule-based analysis rule_based_answer(), so Demo 09 gives a reasonable answer when offline.
"""
...
def answer(question: str, S: dict[str, Any], layout: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        if r:
            return r
    except Exception as e:  # Bad key, network, rate limit → fall back to the rule-based answer; do not let the UI fail
