"""Task location rules (same as frontend/src/simulation/rules.ts):
source / destination must exist in the layout, must not be a charger, and must not be equal. Each TaskType limits the location kinds."""
...
def task_error(loc: dict[str, dict[str, Any]], type_: str, source: str, destination: str) -> Optional[str]:
    """Return an error message; None = valid."""
