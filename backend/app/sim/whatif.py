"""
What-if Simulation (spec 1️⃣7️⃣ / Demo 08)

  Baseline ──clone──▶ run N ticks ──▶ KPI_b
  LIVE state ─┤
  Scenario ──clone──▶ inject ──▶ run N ticks ──▶ KPI_s      →  delta / key events / AI recommendation

The two clones start from the same tick and the same random state. Thus the difference comes only from the injection itself (this is the value of the deterministic engine).
The LIVE engine is not affected.
"""
...
def _window_kpi(eng: SimEngine, start_tick: int, start_completed: int, start_wait: float, start_energy: float, duration: int, counts: dict[str, int]) -> dict[str, Any]:
    ...
    return {
        ...
        "replans": counts.get("ROUTE_REPLANNED", 0),     # Counted from the event stream of the full run; the 500-entry ring does not limit it
...
def run_whatif(base_eng: SimEngine, scen_eng: SimEngine, request: dict[str, Any], start_tick: int) -> dict[str, Any]:
    """Run the What-if. The caller must first clone the live engine twice on the main event loop, then pass the clones in (this prevents concurrent reads and writes with the simulation loop).
    This function touches only these two independent clones, so a worker thread can run it safely."""
    ...
    return {
        ...
        # Extra data (outside the schema, for the frontend comparison table)
...
def recommendation(request: dict[str, Any], base: dict[str, Any] | None, scen: dict[str, Any], delta: dict[str, float]) -> str:
    ...
    text = " ".join(parts)
    # LLM version (optional): a recommendation of two sentences
