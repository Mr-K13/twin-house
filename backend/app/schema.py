"""
Digital Twin Warehouse — Twin State data contract (Pydantic v2)

Maps field-by-field to twin_state.ts. Same rules as the TS file: pure data, serializable, coordinates in meters, tick is simulation time.
All backend API / WebSocket output must pass validation by these models before it goes out.

Usage:
    state = TwinState.model_validate(json_dict)
    state.model_dump_json()            # Send over WebSocket
    copy = state.model_copy(deep=True) # What-if copy
"""
...
class _Base(BaseModel):
    """Shared settings for all models: forbid unknown fields, output enums as strings."""
...
class PerceivedObstacle(_Base):
    """Obstacle that the virtual LiDAR detects (relative to the robot)"""
    kind: Literal["ROBOT", "HUMAN", "RACK"]
    id: Optional[str] = None
    distance_m: float = Field(ge=0)
    bearing_deg: float          # Bearing relative to the heading; left is positive, right is negative


class Perception(_Base):
    """Phase 7: robot perception (270° / 4 m virtual LiDAR) and local obstacle-avoidance state"""
    state: Literal["CLEAR", "SLOWING", "STOPPED", "OFF"] = "OFF"
    ahead_m: float = 4.0        # Clear distance directly ahead (racks included)
...
class RobotState(_Base):
    id: RobotId
    model: str = "AMR-L"
    floor: int = 1                       # Current floor; the y of position is always 0, the renderer adds the floor height
    lift_id: Optional[str] = None        # Lift that the robot rides
...
class LiftState(_Base):
    """Lift (freight lift) — the backend is the only authority; the frontend only interpolates the door/platform animation (spec §2.1/§9.2)"""
    id: str
    state: LiftFsmState = "IDLE"
    floor: Optional[int] = 1             # null while the lift moves (does not belong to any floor)
    target_floor: Optional[int] = None
    y: float = 0                         # Platform height (m); during MOVING the engine interpolates it with smoothstep
    ...
    until_tick: int = 0
    fault: bool = False
    fault_remaining: int = 0             # Remaining action ticks frozen at the fault; on release, the lift continues from here (the platform does not jump)
    trips: int = 0
    busy_ticks: int = 0
...
class HeatmapLayer(_Base):
    ...
    floor: int = 1                # Floor of the data (round-5 fix: one layer per floor, the frontend uses kind:floor as the key)
    cols: int
    rows: int
    values: list[float]
    window_ticks: int
...
class TaskBurst(_Inj):
    ...
    count: int = Field(gt=0, le=30)                  # Public Demo: a maximum of 30 tasks per burst
    priority: TaskPriority = TaskPriority.NORMAL
...
class MsgPatch(_Base):
    ...
    base_tick: int
    tick: int
    patch: dict[str, Any]          # DeepPartial<TwinState>, produced by diff
...
class MsgWhatIfResult(_Base):
    ...
    request_id: Optional[str] = None   # Returns CmdWhatIfRun.request_id
    result: WhatIfResult
...
class MsgError(_Base):
    ...
    code: str
    message: str
    request_id: Optional[str] = None   # Related request, if any (COPILOT_ASK / WHATIF_RUN)
...
class CmdWhatIfRun(_Base):
    ...
    request: WhatIfRequest
    request_id: Optional[str] = Field(default=None, max_length=32)   # Returned unchanged in WHATIF_RESULT / ERROR; the frontend uses it to correlate
...
class VlmObserveBody(_Base):
    """REST /api/vlm/observe: the frontend sends a 512px JPEG data URL (usually 30-80 KB); the base64 limit is 400 KB"""
    camera_id: CameraId
