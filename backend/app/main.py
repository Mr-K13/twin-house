"""
Digital Twin Warehouse — backend (Phase 3)
...
try:
    ...
    load_dotenv()  # backend/.env: OPENAI_API_KEY and other variables
except Exception:
    pass
...
class TwinServer:
    """Holds the engine, the simulation loop, the connections, and the diff state."""
    ...
    def __init__(self, seed: int = 42, db_path: str = "twin.db") -> None:
        ...
        self.last_sent_tick = 0          # Tick of the last broadcast (used as base_tick of PATCH; one round can advance more than one tick)
        self.last_progress = time.monotonic()   # Time of the last successful advance of the simulation loop (used by health)
        self.loop_errors = 0
    ...
    async def run(self) -> None:
        ...
        while True:
            ...
            try:
                ...
                raise
            except Exception as e:  # A failure in one round (for example, a temporary SQLite write error) must not stop the full simulation
    ...
    async def after_ticks(self) -> None:
        ...
        except Exception as e:  # The DB is only for audit; a failure must not affect the live stream
        ...
        # base_tick = the last tick really sent. When one round advances more than one tick (10x speed or a slow host), the frontend then does not see a false packet loss and RESYNC.
        ...
        if tick % HEATMAP_EVERY == 0:
            for fl in eng.traffic:   # One per floor (round-5 fix)
    ...
    def _robot_patch(self) -> dict[str, Any]:
        """For each robot, send only the fields that changed (compared with the last send). Send path only when it changes. Round the floats. The frontend merges with {...prev, ...patch}."""
        out: dict[str, Any] = {}
    ...
    async def handle(self, ws: WebSocket, raw: str) -> None:
        ...
        if not ok:
            # The message is not validated yet, but try to extract request_id and return it, so the frontend can relate the error to a pending Copilot / What-if
            rid = None
            try:
        ...
        if t == "RESYNC":
            ...
            # Run the LLM call in a thread, so it does not block the simulation loop
    ...
    async def run_whatif_safe(self, req: dict[str, Any]) -> dict[str, Any]:
        """First clone on the main event loop (no tick is in progress then), then give the independent clones to a worker thread. Only one What-if can run at a time."""
...
async def lifespan(app: FastAPI):
    ...
    if server._task and not server._task.done():
        server._task.cancel()
    server.last_progress = time.monotonic()   # A fresh start does not count as a stall
...
app = FastAPI(title="Digital Twin Warehouse", version="0.3.0", lifespan=lifespan)
# CORS: fully open by default for local development. For deployment, set the frontend domains with TWIN_CORS_ORIGINS (comma-separated), for example https://your-app.vercel.app
...
class GuardMiddleware:
    """Pure ASGI middleware (not BaseHTTPMiddleware, so it can intercept at the receive layer and return 413 directly):
    For requests that change state, check the Origin and count the body size as the bytes really received. GET is not affected."""
    ...
    async def __call__(self, scope: dict[str, Any], receive: Any, send: Any) -> None:
        ...
        async def capped_receive() -> dict[str, Any]:
            ...
            if msg["type"] == "http.request":
                ...
                if total > MAX_BODY_BYTES:
                    tripped = True
                    # Stop the read: return "end of body" so the downstream JSON parse fails, then guarded_send changes the response to 413
        ...
        async def guarded_send(msg: dict[str, Any]) -> None:
            ...
            if tripped:
                ...
                return   # Discard the original downstream response
            await send(msg)
...
async def ws_endpoint(ws: WebSocket) -> None:
    ...
    try:
        ...
        for fl in server.engine.traffic:   # One per floor (round-5 fix)
...
async def health() -> Any:
    """If the simulation loop is dead, or made no advance for more than STALL_S → 503, so Render restarts the service. A DB failure is only reported, not counted as a failure (audit only)."""
    h = health_payload()
    if not h["ok"]:
...
async def validate_state() -> dict[str, Any]:
    """Validate with Pydantic that the current state matches the contract (for debug)."""
...
async def post_vlm(body: VlmObserveBody, request: Request) -> dict[str, Any]:
    """The frontend sends the Live Camera frame (JPEG data URL). Return a VlmObservation and write it to cameras[id].last_observation."""
    throttle(request, "ai")
...
async def post_whatif(body: dict[str, Any], request: Request) -> dict[str, Any]:
    """Clone the LIVE engine, inject the scenario, run duration_ticks, and return the Baseline vs Scenario comparison. LIVE is not affected."""
