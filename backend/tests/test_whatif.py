"""Phase 6: What-if — clone determinism, LIVE not affected, scenario differs, endpoint contract"""
import json
...
def test_clone_is_deterministic_and_independent():
    ...
    assert json.dumps(e.state, sort_keys=True) == snap           # LIVE did not change
...
def test_whatif_compound_scenario_hurts_task_time():
    ...
    # (No assertion on throughput: TASK_BURST adds more tasks, so the completed count can exceed baseline; the key metric is the worse avg task time above)
