"""FastAPI / WebSocket protocol tests."""
import json
...
def test_health_reports_dead_sim_task():
    """When the sim task dies → /api/health returns 503 (so Render restarts). The next TestClient's lifespan restarts the loop."""
    import time as _t
    with TestClient(app) as client:
...
def test_patch_base_tick_chains_across_multi_tick_rounds():
    """A PATCH's base_tick must equal the previous PATCH's tick (even when one round advances multiple ticks)."""
    with TestClient(app) as client:
...
def test_sim_loop_survives_db_failure(monkeypatch):
    """When a SQLite write raises an exception, the sim loop must continue to advance (only record loop_errors) and must not die."""
    import time as _t
