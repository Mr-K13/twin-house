"""Phase 5: Copilot and VLM (no API key needed: tests the fallback and the endpoint contract; with a key, ask_llm is actually called)"""
import os
...
def test_vlm_simulated_observation_matches_ground_truth():
    e = _engine_with_faults()
    o = observe("CAM-B02", None, e.state, L)       # Zone B has a human
    assert o["event"] == "human_detected" and o["blocked"] and o["bbox"]
    o2 = observe("CAM-C01", None, e.state, L)      # Zone C has no human
