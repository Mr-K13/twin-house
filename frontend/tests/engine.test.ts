...
describe("A*", () => {
  ...
  it("finds a path between two shelf access points and never crosses obstacles", () => {
    ...
    // Consecutive grid cells must be adjacent
...
describe("SimEngine", () => {
  ...
  }, 30000);   // 12000 ticks can exceed the default 5 s under parallel test load -- set an explicit limit (same as Demo 10)
...
describe("multi-floor", () => {
  it("robot rides a lift to complete a cross-floor task", () => {
    const eng = new SimEngine(layout, { seed: 7 });
    // Create only one cross-floor task: floor-2 shelf → floor-1 packing station (R17–R20 are on floor 2, but dispatch can select any robot)
  ...
  it("floor-2 grid only covers the mezzanine footprint", () => {
    ...
    expect(at(51.5, 44.5)).toBe(1);      // lift shaft = obstacle (round-8: cabin entry and exit use microMove, not the grid)
    expect(at(20.5, 47.5)).not.toBe(1);  // mezzanine walkway
    expect(at(80, 20)).toBe(1);          // outside footprint = no floor slab
  });
});
...
describe("lift state machine (spec §9–§14)", () => {
    ...
    for (let i = 0; i < 24000 && t.status !== "COMPLETED"; i++) {
      ...
      for (const L of Object.values(eng.state.lifts)) {
        if (L.state === "MOVING_UP" || L.state === "MOVING_DOWN") {
          ...
          expect(L.floor).toBeNull();   // a moving lift belongs to no floor
        }
      }
      for (const r of Object.values(eng.state.robots)) {
        const prev = floorAt[r.id] ?? r.floor;
        if (r.floor !== prev) {
          // The floor flips only when the ALIGHTING stage ends, and the robot must be out of the cabin (> 1.4 m from every cabin center)
  ...
  it("clearing a lift fault resumes the platform from the frozen height — no teleport", () => {
    const { eng, t } = boot();
    // Wait until a robot boards a lift and the lift starts to move
    ...
    // Inject the fault while the lift moves
    for (let i = 0; i < 20 && (L.state === "MOVING_UP" || L.state === "MOVING_DOWN") && Math.abs(L.y) < 4; i++) eng.step();
    eng.inject({ kind: "LIFT_FAULT", lift_id: lid! });
    eng.step();
    const frozenY = L.y;
    for (let i = 0; i < 200; i++) { eng.step(); expect(L.y).toBe(frozenY); }   // height stays frozen during the fault
    eng.clearInjection("LIFT_FAULT", lid!);
    let prevY = L.y;
    for (let i = 0; i < 2000; i++) {
      eng.step();
      expect(Math.abs(L.y - prevY), "platform teleported after fault clear").toBeLessThan(0.5);   // movement per tick must be smooth
      prevY = L.y;
    }
  ...
  it("lift fault reroutes waiting robots to the other lift; rider is never teleported", () => {
    const { eng, t } = boot();
    // Wait until the robot enters the lift flow
    ...
    if (!r.lift_id) {
      // Not boarded: the robot must use the other lift
...
describe("rehydration — LOCAL engine takes over after WS disconnect (round-6 P1)", () => {
  const snapshotOf = (e: SimEngine) => JSON.parse(JSON.stringify(e.state));

  it("a moving robot does not arrive instantly (NAVIGATING must not become PICKING on the next tick)", () => {
    ...
    expect(r1.fsm).not.toBe("PICKING");            // old bug: rt.target reset to zero → robot counted as arrived immediately
    expect(r1.path.length).toBeGreaterThan(0);     // the path is still in use, not cleared
  }, 60000);

  it("task / event sequence numbers continue; no duplicate ID appears", () => {
  ...
  it("handover during a lift ride: the trip continues, the floor flips only after ALIGHTING completes, and the task completes", () => {
...
describe("dispatch audit ↔ actual lift (round-6 P2)", () => {
  it("the lift in the dispatch audit = the lift the robot actually queues for", () => {
...
describe("rehydration edge cases (round-7)", () => {
  ...
  it("handover during loaded obstacle avoidance: robot returns to TRANSPORTING, does not pick again, does not repeat TASK_STARTED", () => {
    ...
    expect(rid).toBeTruthy();
    // The traffic congestion injection sets all moving robots to OBSTACLE_DETECTED (same as the production path)
    ...
    b.step();
    // old bug: phase could not be derived → REPLANNING fell back to NAVIGATING → robot did PICKING again on arrival
    ...
    // After handover (event sequence > maxE), no new TASK_STARTED for this task is permitted (= second picked item)
    const dup = b.state.recent_events.filter((e) => parseInt(e.id.slice(1), 10) > maxE && e.type === "TASK_STARTED" && e.task_id === tid);
    expect(dup).toHaveLength(0);
  }, 120000);

  it("handover during delivery with a conveyor fault: remaining time includes stationSlowdown; delivery does not complete early", () => {
    ...
    expect(rid).toBeTruthy();   // just entered DELIVERING (elapsed ≤ 1); true remaining ≈ DROP_TICKS × 4 = 120
    const b = new SimEngine(layout, { seed: 32, initialState: snapshotOf(a) });
    let n = 0;
    while (b.state.robots[rid!].fsm === "DELIVERING" && n < 300) { b.step(); n++; }
    expect(n).toBeGreaterThan(30);      // old bug: no stationSlowdown factor → delivery completed in ≤ 30 ticks
    expect(n).toBeLessThanOrEqual(121);
  }, 120000);

  it("after handover, avg_task_time_s keeps the snapshot value and does not reset to zero", () => {
    const a = new SimEngine(layout, { seed: 33 });
    for (let i = 0; i < 60000 && a.state.kpi.operation.completed_today < 5; i++) a.step();
    while (a.state.sim.tick % 10 !== 0) a.step();   // align with the KPI update
    const snap = snapshotOf(a);
    ...
    for (let i = 0; i < 10; i++) b.step();          // step across at least one KPI update
    if (b.state.kpi.operation.completed_today === done0) {
      expect(b.state.kpi.operation.avg_task_time_s).toBe(avg0);   // old bug: empty taskTimes → 0
    } else {
      expect(b.state.kpi.operation.avg_task_time_s).toBeGreaterThan(0);   // a new task just completed: the value must not reset to zero
    }
  }, 120000);
});

describe("lift shaft as nav obstacle (round-8)", () => {
  it("both floor grids block the shaft 3×5 (x ±1.4 / z ±1.9, covers the 3.6 m shaft depth); all queue cells and exit candidates stay walkable", () => {
    for (const fl of [1, 2]) {
      ...
      for (const l of layout.lifts) {
        ...
        // round-9g: also block the north and south faces (cell±2 rows) -- the shaft is 3.6 m deep; with only 3 rows blocked, the guard mesh extends into walkable cells and passing robots clip the posts
        ...
        expect(g.cells[l.cell[1] * g.cols + (l.cell[0] - 2)], `${l.id} gate cell F${fl}`).not.toBe(1);   // gate relay cell
  ...
  it("A* paths never cross the shaft; robots outside the lift flow never appear in the shaft area", () => {
    // mezzanine support columns (round-9c): blocked on F1; paths and robot bodies must not cross a column
    ...
    // round-9g: the four body corners (OBB) must not enter the "visual" shaft rectangle (W 2.8 × D 3.6) -- a center-point check misses edge scrapes
...
describe("alighting exits through the gate (round-8d three stages + OBB)", () => {
  it("engine gate geometry constants match the 3D shaft model (prevents drift between the two)", () => {
    expect(SIM.LIFT_SHAFT_HALF_X * 2).toBe(LIFT_SHAFT.W);
    expect(SIM.LIFT_DOOR_HALF_W).toBe(LIFT_SHAFT.LEAF);
  });

  it("three-stage alighting: per-frame OBB corner gate check + no large turn while moving; verifies both directions and loaded/empty robots", () => {
    const eng = new SimEngine(layout, { seed: 5 });
    const tasks = [
    ...
    for (let i = 0; i < 90000; i++) {
      ...
      for (const r of Object.values(eng.state.robots)) {
        ...
        // OBB corners (actual rotated robot body)
        ...
        // When the body crosses the door plane, each crossing edge must intersect x=doorPlane at a z inside the door opening
        ...
        // No large turn while moving: when heading changes > 0.09 rad in one tick, movement must be < 0.02 m (turn in place)
        const pv = prev[r.id];
        if (pv) {
        ...
        // round-9b: the rotated body (OBB) of an alighting robot must not intersect a queued or approaching robot (same physical floor)
...
describe("lift exit faces the destination (round-8e)", () => {
  it("the exit is on the destination side, never the opposite side with a detour (both lifts, both floors)", () => {
    const eng = new SimEngine(layout, { seed: 1 });
...
describe("building pillars & charging dock (round-9d)", () => {
  it("building pillars are in layout.obstacles, block the grid, and do not sit on a conveyor", () => {
    ...
    for (const o of obs) {
      ...
      if (o.kind !== "PILLAR") continue;   // CONVEYOR_EQUIP (round-9f endpoint machines) sits at conveyor endpoints by design
  ...
  it("conveyor endpoint machines block the grid, vertical conveyors sit against the wall (no wall-side narrow lane), inner lanes stay clear, simulation keeps robots out of machine areas (round-9f)", () => {
    const eq = (layout.obstacles ?? []).filter((o) => o.kind === "CONVEYOR_EQUIP");
    expect(eq.length).toBeGreaterThan(0);
    for (const cv of layout.conveyors)   // Each conveyor must have a machine footprint at both ends (single data source: endpoint ±1.5 m)
    ...
    for (let r = 34; r <= 60; r++) {   // vertical conveyor body: fully blocked between conveyor and wall (no wall-side narrow lane)
    ...
    for (let r = 37; r < 58; r++) {    // conveyor side outside machine areas: inner lane stays clear (west 4 cells, east 5 cells)
    ...
    for (let r = 33; r < 37; r++)      // all three crossings over z=35 (west/center/east) must stay open -- traffic must not all funnel into the central aisle at the lift lobby
  ...
  it("a charging robot stops exactly at the charging pad center, faces the charger, and does not overlap another robot", () => {
    ...
    for (let i = 0; i < 20000; i++) {
      ...
      if (chg.length >= 2) { expect(chg.length).toBeGreaterThanOrEqual(2); return; }   // round-9e: two robots must charge at the same time
    }
