...
describe("lift lobby congestion (fix for alighting and queued robots that block each other)", () => {
  it("many cross-floor tasks in both directions all complete; nobody starves at the lobby", () => {
    ...
    // Track the number of consecutive ticks each robot stays still during the lift flow
    ...
    for (let i = 0; i < 90000 && !tasks.every((t) => ["COMPLETED", "TRANSFERRED", "FAILED"].includes(t.status)); i++) {
      ...
      for (const r of Object.values(eng.state.robots)) {
        ...
        // ALIGHTING / BOARDING must never stay stuck for more than 60 seconds
