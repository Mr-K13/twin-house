...
export function TaskQueuePanel() {
  ...
  // Match the reference image: 3 rows in progress + 2 rows waiting
...
export function ThroughputPanel() {
  ...
  // x axis: the last 120 minutes of simulation time (1 minute per point); the y axis maximum grows with the data
  const n = 120;
    ...
    <Panel title="Throughput" sub="(Today)">
      ...
      <div className="chart-wrap" style={{ marginTop: 4 }}>
        <svg viewBox={`0 0 ${W} ${H}`} width="100%" height="100%" preserveAspectRatio="none" style={{ display: "block" }}>
          ...
          {ySteps.map((v) => (
            <g key={v}><line x1={px} x2={W - 8} y1={sy(v)} y2={sy(v)} stroke="#e2e8f0" strokeWidth="1" /><text x={px - 6} y={sy(v) + 4} fill="#64748b" fontSize="9" textAnchor="end" fontFamily="JetBrains Mono, monospace">{v}</text></g>
          ))}
          {xLabels.map((l, i) => <text key={i} x={l.x} y={H - 8} fill="#64748b" fontSize="9" textAnchor="middle" fontFamily="JetBrains Mono, monospace">{l.label}</text>)}
    ...
    <Panel title="Robot Status">
      <div className="donut-wrap">
        <svg viewBox="0 0 130 130">
          <circle cx="65" cy="65" r={R} fill="none" stroke="#e2e8f0" strokeWidth={R - r} />
          {parts.map((p) => {
          ...
          <text x="65" y="62" textAnchor="middle" fill="#1e293b" fontSize="22" fontWeight="700" fontFamily="JetBrains Mono, monospace">{f.total}</text>
