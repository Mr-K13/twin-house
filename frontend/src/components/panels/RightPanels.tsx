...
/** Information panel for the selected lift (spec §18) */
...
export function LiveCameraPanel() {
  ...
  const busyRef = useRef(false);   // The setInterval closure captures the old busy state; only a ref can block overlapping requests
  ...
  const analyze = async () => {
    ...
    try {
      // Capture the Live Camera canvas (preserveDrawingBuffer is necessary), scale it to a 512px JPEG, and send it to the VLM
