...
function Conveyor({ c }: { c: LayoutConveyor }) {
  ...
  // Polyline parameterization: total length and each segment start, so parcels can move along the full path (corners included)
  ...
  // Parcels sit in a world-coordinate group (no segment rotation) and advance along the polyline each frame
  const parcels = (
  ...
  // round-9e: end equipment -- infeed hopper at the start, receiving station at the end (parcels no longer appear to fall to the floor)
