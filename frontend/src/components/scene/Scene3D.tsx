...
/** Camera focus animation: when focusTarget changes, move the OrbitControls target smoothly */
  ...
  useEffect(() => {
    if (selected && selected !== prevSel.current) {
      ...
      if (r) { const ey = FLOOR_ELEV[r.floor] ?? 0;
        goal.current = new THREE.Vector3(r.position[0], ey + 0.5, r.position[2]); // Look along the aisle direction (x axis) so a rack in front does not block the view
...
/** Background, fog, and procedural environment map (RoomEnvironment, no HDR download, works offline) */
...
export function SceneContent({ quality, lite = false }: { quality: "low" | "medium" | "high"; lite?: boolean }) {
  ...
  const activeFloor = lite ? "all" : af;   // The lite scene for CCTV / thumbnails always shows all floors
