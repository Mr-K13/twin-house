...
/** Zone neon floor border + floating label; BLOCKED is red, CONGESTED is orange */
export function ZoneOverlay({ labels = true }: { labels?: boolean }) {
  ...
  const activeFloor = !labels || af === "exploded" ? "all" : af;   // The lite (CCTV) scene shows all floors
  return (
    <group>
      {layout.zones.filter((z) => activeFloor === "all" || (z.floor ?? 1) === activeFloor).map((z) => {
        ...
        const ey = (FLOOR_ELEV[z.floor ?? 1] ?? 0) + (labels && af === "exploded" && (z.floor ?? 1) === 2 ? 5 : 0);   // In the lite (CCTV, labels=false) scene the platform does not rise, so the zone border must not rise either
        ...
        // Put the label at the outer corner of the zone (A top-left, B top-right, C bottom-left, D bottom-right)
