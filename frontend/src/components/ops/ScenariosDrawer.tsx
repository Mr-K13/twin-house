/**
 * Failure injection drawer (spec 1️⃣3️⃣): Robot Failure / Conveyor Failure / Sensor(Camera) Failure / Human Intrusion / Traffic Congestion / Task Burst
 * The bottom lists the currently active injections (derived from TwinState, no extra state); one click clears an injection.
 */
...
export function ScenariosDrawer() {
  ...
  // Currently active injections (derived from state)
    ...
    <aside className="drawer" role="dialog" aria-label="Scenario Injection" ref={trap} tabIndex={-1}>
      ...
      <div className="drawer-b">
        ...
        <Row title="Human Intrusion" demo="03" desc="A worker enters the zone → zone blocked → robots re-plan">
          ...
          <input type="number" min={10} max={600} value={duration} onChange={(e) => setDuration(+e.target.value)} title="seconds" /><span className="unit">s</span>
